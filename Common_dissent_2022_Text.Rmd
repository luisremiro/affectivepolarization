...
df <- read_csv("dades/cses_imd.csv")
...
donde \[{g}\] es el grupo ajeno (electores o líderes), \[{i}\] cada individuo \[{{Like}_{max i}}\] el valor agregado asignado al grupo con el que se identifica el individuo, es decir, el mayor agrado (10) y \[{{Like}_{ig}}\] el valor de agrado hacia a cada out-group \[{g}\]. Por último, \[{Vg}\] es el tamaño de cada partido. Esta medida puede ser:
1. Los resultados de las elecciones más recientes.
2. El promedio de intención de voto de cada partido.
...
#Dependant variable: Party identification
#IV: AP measures
